...
def create_langchain_messages(
    history: list[Message], current_message: str
...
async def generate_streaming_response(
    ...
    try:
        ...
        async for msg, metadata in llm.astream(
            {"messages": messages}, thread_id=conversation_id
        ):
            if await request.is_disconnected():
            ...
            try:
                ...
                logger.critical(
                    f"Critical error processing chunk {chunk_count}: {chunk_error}"
                )
                continue
        ...
        logger.info(
            f"Streaming completed for conversation {conversation_id}, total chunks: {chunk_count}"
        )
        ...
        logger.critical(
            f"Error generating streaming response ({error_type}): {error_message}"
        )
        ...
        if llm.checkpointer.get({"configurable": {"thread_id": conversation_id}}):
            ...
            logger.info(
                f"LangGraph State of Conversation ID {conversation_id} has been initialized."
            )
        ...
        if not (await request.is_disconnected()):
            ...
            logger.error(
                f"Connection was closed before sending completion signal for conversation id {conversation_id}."
            )
